...
export default function TutorPage() {
  ...
  const load = async () => {
    ...
    setFeed(
      (await api<{ items: FeedItem[] }>("/api/feed/guardians", {}, accessToken))
        .items,
    );
    setConversations(
      (
  ...
  useEffect(() => {
    if (!loading && (!user || user.activeRole !== "tutor"))
      router.replace(user?.activeRole === "guardian" ? "/guardian" : "/");
  }, [loading, user, router]);
  useEffect(() => {
    void load();
  }, [accessToken]);
    ...
    <main className="container grid">
      <div className="row">
        <div>
          ...
          <p className="small">
            Welcome {user.tutorProfile?.displayName || user.email}
          </p>
        </div>
        <div className="row">
      ...
      {!user.tutorProfile?.profileCompleted ? (
        <div className="card">
          <p>
            Tutor role exists, but tutor profile is still incomplete. Later you
            can expand this before showing recommendations.
          </p>
        </div>
      ) : null}
      {showAdd ? (
        <form
          ...
          onSubmit={async (e) => {
            e.preventDefault();
            await addRole("guardian", {
              displayName: name,
              primaryLocation: location,
            });
          }}
        >
          <h2>Add guardian role</h2>
          <input
            className="input"
            value={name}
          ...
          <input
            ...
            required
          />
          <button className="btn primary">Create guardian role</button>
        </form>
      ) : null}
      <div className="grid2">
        <section className="card">
          <h2>Mock guardian feed</h2>
          <div className="grid">
            {feed.map((item) => (
